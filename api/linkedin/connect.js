...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      "https://api49.unipile.com:17927/api/v1/hosted/accounts/link",
      {
        method: "POST",
        headers: {
          "X-API-KEY": apiKey,
    ...
    if (!response.ok) {
      ...
      return res
        ...
        .json({ error: "Failed to create Unipile auth link", details: data });
    }
    ...
    return res
      .status(500)
      .json({ error: "LinkedIn connect failed", details: err.message });
  }
}
