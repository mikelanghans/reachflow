...
export function useSupabaseData(agencyId) {
    ...
    try {
      ...
      setCampaignsState(
        (campaignData || []).map((c) => ({
          id: c.id,
          name: c.name,
          client: clientMap[c.client_id] || c.client_id,
      ...
      setLeadsState(
        (leadData || []).map((l) => ({
          ...
          company: l.company,
          initials: l.initials,
          color: l.color || "#7d8590",
          clientColor: "#58a6ff",
          campaign: l.campaign_id,
          client_id: l.client_id,
          client: clientMap[l.client_id] || l.client_id,
          pipelineStage: l.pipeline_stage,
          days: l.days_in_stage,
          status: l.status,
          unread: l.unread,
          last: l.last_activity_at ? "recently" : "new",
          messages: messagesByLead[l.id] || [],
          triggerKeyword: l.trigger_keyword,
          triggerPost: l.trigger_post,
  ...
  const addClient = useCallback(
    async (client) => {
      const { data, error } = await supabase
        ...
        .single();
      if (data) {
        setClientsState((cs) => [...cs, { ...client, id: data.id }]);
        logActivity("client", `New client added: ${client.name}`);
      }
    },
  ...
  const addCampaign = useCallback(
    async (campaign) => {
      const { data, error } = await supabase
        ...
        .insert({
          agency_id: agencyId,
          client_id: campaign.client_id || campaign.clientId || null,
          name: campaign.name,
          status: "active",
      ...
      if (data) {
        ...
        setCampaignsState((cs) => [...cs, newCampaign]);
  ...
  const toggleCampaign = useCallback(
    async (id) => {
      ...
      setCampaignsState((cs) =>
        cs.map((c) => (c.id === id ? { ...c, status: newStatus } : c)),
      );
    },
    [campaigns],
  );
  ...
  const toggleReviewMode = useCallback(
    async (id) => {
      ...
      await supabase
        .from("campaigns")
        .update({ review_mode: newMode })
        .eq("id", id);
      setCampaignsState((cs) =>
        cs.map((c) => (c.id === id ? { ...c, reviewMode: newMode } : c)),
      );
    },
    [campaigns],
  );
    ...
    async (updaterOrArray) => {
      ...
      if (newLeads.length > 0) {
        const rows = newLeads.map((l) => ({
          ...
          initials: l.initials,
          color: l.color,
          pipeline_stage: l.pipelineStage || "prospecting",
          status: l.status || "pending",
          campaign_id: l.campaign_id || null,
          client_id: l.client_id || null,
          trigger_keyword: l.triggerKeyword,
          trigger_post: l.triggerPost,
          linkedin_urn: l.linkedin_urn || null,
  ...
  const updateLeadsBulk = useCallback(async (leadIds, updates) => {
    ...
    if ("pipelineStage" in updates)
      dbUpdates.pipeline_stage = updates.pipelineStage;
    ...
    setLeadsState((ls) =>
      ls.map((l) => (leadIds.includes(l.id) ? { ...l, ...updates } : l)),
    );
  }, []);

  // Permanently deletes leads from Supabase. Bulk delete in the UI was
  // previously local-state-only (same class of bug as the brand-save and
  // bulk-status issues) — deleted leads would reappear after any refresh
  // since they were never actually removed from the database.
  const deleteLeadsBulk = useCallback(async (leadIds) => {
    const { error } = await supabase.from("leads").delete().in("id", leadIds);
    if (error) {
      console.error("deleteLeadsBulk failed:", error);
      throw error;
    }
    setLeadsState((ls) => ls.filter((l) => !leadIds.includes(l.id)));
  }, []);

  const updateLeadStage = useCallback(async (id, pipelineStage) => {
    await supabase
      .from("leads")
      .update({ pipeline_stage: pipelineStage, days_in_stage: 0 })
      .eq("id", id);
    setLeadsState((ls) =>
      ls.map((l) => (l.id === id ? { ...l, pipelineStage, days: 0 } : l)),
    );
  }, []);

  const sendMessage = useCallback(
    async (leadId, body, channel = "linkedin") => {
  ...
  const bookMeeting = useCallback(
    async (leadId) => {
      ...
      if (lead)
        logActivity(
          "meeting",
          `Meeting booked with ${lead.name} · ${lead.company}`,
        );
    },
    [leads, logActivity],
  );
  ...
  const saveVaryMessages = useCallback(
    async (enabled) => {
      const { error } = await supabase
        .from("agencies")
        .update({
