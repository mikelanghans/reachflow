...
function Badge({ status }) {
  ...
  return (
    <span style={{ background: c + "22", color: c, border: `1px solid ${c}40`, fontSize: 12.5, fontWeight: 700, padding: "3px 9px", borderRadius: 4, letterSpacing: "0.04em", whiteSpace: "nowrap" }}>
...
function useToasts() {
  ...
  useEffect(() => {
    ...
    return () => { _toastListeners = _toastListeners.filter(l => l !== setToasts); };
  }, []);
  return toasts;
}
...
function ToastContainer() {
  ...
  return (
    <div style={{ position: "fixed", bottom: "1.5rem", right: "1.5rem", zIndex: 500, display: "flex", flexDirection: "column", gap: 8 }}>
      {toasts.map(t => (
        <div key={t.id} style={{
          background: t.type === "success" ? T.card : t.type === "warn" ? T.card : "#1a0a0a",
          border: `1px solid ${t.type === "success" ? T.accent : t.type === "warn" ? T.yellow : T.red}`,
          borderRadius: 10, padding: "11px 16px", fontSize: 13, fontWeight: 600, maxWidth: 340,
          color: t.type === "success" ? T.accent : t.type === "warn" ? T.yellow : T.red,
          boxShadow: "0 4px 24px rgba(0,0,0,0.5)",
          animation: "slideIn 0.2s ease",
        }}>
          {t.type === "success" ? "✓ " : t.type === "warn" ? "⚠ " : "✕ "}{t.message}
        </div>
      ))}
...
async function apiFetch(url, opts = {}, errorMsg = "Request failed") {
  try {
    ...
    pushToast(err.message === "Failed to fetch" ? "Connection error — check your internet" : err.message || errorMsg);
    return null;
  }
}
...
function Skeleton({ w = "100%", h = 16, br = 6, mb = 0 }) {
  return (
    <div style={{ width: w, height: h, borderRadius: br, background: T.faint, marginBottom: mb, animation: "shimmer 1.5s infinite", overflow: "hidden", position: "relative" }}>
      <div style={{ position: "absolute", inset: 0, background: `linear-gradient(90deg, transparent, ${T.border}88, transparent)`, animation: "shimmerMove 1.5s infinite" }} />
  ...
  return (
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "1.125rem" }}>
      <div style={{ display: "flex", gap: 10, marginBottom: "0.875rem" }}>
        ...
        <div style={{ flex: 1 }}><Skeleton h={14} mb={6} /><Skeleton w="60%" h={11} /></div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: "0.75rem" }}>
        {[0,1,2].map(i => <Skeleton key={i} h={44} br={7} />)}
      </div>
      <Skeleton h={32} br={7} />
    </div>
  );
  ...
  return (
    <tr>
      {[100, 120, 90, 130, 60, 70, 80].map((w, i) => (
        <td key={i} style={{ padding: "11px 14px", borderBottom: `1px solid ${T.faint}` }}>
          <Skeleton w={w} h={13} />
        </td>
      ))}
    </tr>
  ...
  return (
    <div style={{ position: "relative" }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ background: "transparent", border: "none", cursor: "pointer", padding: "6px 8px", borderRadius: 8, display: "flex", alignItems: "center", gap: 6, color: T.muted, position: "relative" }}>
        <span style={{ fontSize: 16 }}>🔔</span>
        {unreadCount > 0 && (
          <span style={{ position: "absolute", top: 2, right: 2, background: T.red, color: "#fff", fontSize: 9, fontWeight: 800, width: 16, height: 16, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {open && (
        <>
          ...
          <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, width: 320, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 99, overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.875rem 1rem", borderBottom: `1px solid ${T.border}` }}>
              <div style={{ color: T.text, fontSize: 14, fontWeight: 700 }}>Notifications {unreadCount > 0 && <span style={{ background: T.red, color: "#fff", fontSize: 10, fontWeight: 800, padding: "1px 6px", borderRadius: 8, marginLeft: 6 }}>{unreadCount}</span>}</div>
              {unreadCount > 0 && <button onClick={markAllRead} style={{ background: "transparent", border: "none", color: T.muted, fontSize: 11, cursor: "pointer" }}>Mark all read</button>}
            </div>

            <div style={{ maxHeight: 380, overflowY: "auto" }}>
              {activity.length === 0 && (
                <div style={{ padding: "2.5rem", textAlign: "center", color: T.muted, fontSize: 13 }}>No activity yet</div>
              )}
              {activity.slice(0, 20).map((a, i) => {
                const cfg = NOTIF_ICONS[a.type] || { icon: "·", color: T.muted };
                const isUnread = !readIds.includes(a.id);
                return (
                  <div key={a.id} onClick={() => { onNavigate(navMap[a.type] || "dashboard"); setReadIds(ids => [...new Set([...ids, a.id])]); setOpen(false); }}
                    style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "0.75rem 1rem", borderBottom: i < activity.slice(0,20).length - 1 ? `1px solid ${T.faint}` : "none", cursor: "pointer", background: isUnread ? T.accentDim : "transparent", transition: "background 0.1s" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ color: T.text, fontSize: 12, lineHeight: 1.5, fontWeight: isUnread ? 600 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.message}</div>
                      <div style={{ color: T.faint, fontSize: 11, marginTop: 2 }}>{timeAgo(a.time)}</div>
                    </div>
                    {isUnread && <div style={{ width: 7, height: 7, borderRadius: "50%", background: T.accent, flexShrink: 0, marginTop: 4 }} />}
                  </div>
                );
              })}
            </div>
...
function AuditLogModal({ client, activity, onClose }) {
  ...
  const filtered = activity.filter(a =>
    (a.meta?.clientName === client.name || a.meta?.name?.includes(client.name) || a.message?.includes(client.name)) &&
    (typeFilter === "all" || a.type === typeFilter)
  );

  const exportCSV = () => {
    const rows = [["Time", "Type", "Event"]];
    filtered.forEach(a => rows.push([new Date(a.time).toLocaleString(), a.type, a.message]));
    const csv = rows.map(r => r.map(v => `"${(v||"").replace(/"/g,'""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const el = document.createElement("a"); el.href = url; el.download = `${client.name}-activity.csv`; el.click();
  ...
  const TYPES = ["all", "reply", "meeting", "pipeline", "import", "campaign", "client", "flow"];
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 150, padding: "1rem" }}>
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, width: "100%", maxWidth: 600, maxHeight: "85vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "1.25rem 1.5rem", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ color: T.text, fontSize: 16, fontWeight: 700 }}>{client.name} — Activity Log</div>
            </div>
            <div style={{ color: T.muted, fontSize: 12, marginTop: 2, marginLeft: 42 }}>{filtered.length} events</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={exportCSV} style={{ background: T.card, color: T.muted, border: `1px solid ${T.border}`, borderRadius: 7, padding: "7px 12px", cursor: "pointer", fontSize: 12 }}>↓ Export CSV</button>
            <button onClick={onClose} style={{ background: "transparent", border: "none", color: T.muted, cursor: "pointer", fontSize: 20 }}>×</button>
          </div>
        </div>

        <div style={{ padding: "0.75rem 1.5rem", borderBottom: `1px solid ${T.border}`, display: "flex", gap: 6, flexWrap: "wrap", flexShrink: 0 }}>
          {TYPES.map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} style={{ background: typeFilter === t ? T.accentBg : "transparent", color: typeFilter === t ? T.accent : T.muted, border: `1px solid ${typeFilter === t ? T.accent : T.border}`, borderRadius: 5, padding: "3px 9px", cursor: "pointer", fontSize: 11, fontWeight: typeFilter === t ? 700 : 400 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {filtered.length === 0 ? (
            <div style={{ padding: "3rem", textAlign: "center", color: T.muted, fontSize: 13 }}>No activity found for this client</div>
          ) : filtered.map((a, i) => {
            const cfg = NOTIF_ICONS[a.type] || { icon: "·", color: T.muted };
            return (
              <div key={a.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "0.875rem 1.5rem", borderBottom: i < filtered.length - 1 ? `1px solid ${T.faint}` : "none" }}>
                <div style={{ width: 28, height: 28, borderRadius: 7, background: cfg.color + "22", color: cfg.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0, marginTop: 1 }}>{cfg.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: T.text, fontSize: 13, lineHeight: 1.5 }}>{a.message}</div>
                  <div style={{ color: T.faint, fontSize: 11, marginTop: 3 }}>{timeAgo(a.time)}</div>
                </div>
              </div>
            );
          })}
...
// ─── WELCOME ONBOARDING ───────────────────────────────────────────────────────
const WELCOME_STEPS = [
  { icon: "◎", title: "Welcome to ReachFlow", body: "Your agency's outreach platform. You're starting with a clean slate — no demo data, just your real clients and campaigns. Here's a quick tour of what to do first.", cta: "Let's go →" },
  { icon: "👥", title: "Add your first client", body: "Every outreach campaign runs under a client. Click '+ Add Client' in the sidebar to kick off the onboarding wizard — it takes about 3 minutes and connects their LinkedIn account via OAuth.", cta: "Got it →" },
  { icon: "◉", title: "Import leads", body: "Go to Lead Lists → + Add Leads. Use the Search LinkedIn tab to find prospects directly in the app, or paste a LinkedIn search URL. Leads are scored against your ICP automatically.", cta: "Got it →" },
  { icon: "⑃", title: "Build your sequence", body: "In Campaigns, create a new campaign and open the Flow Builder. Build a connection request → follow-up sequence, or use the Social Warming template for a warm approach over 4 days.", cta: "Got it →" },
  { icon: "🚀", title: "You're ready to go", body: "Once a client's LinkedIn is connected and leads are imported, the scheduler sends messages automatically within your configured time window. Check the inbox for replies.", cta: "Start →" },
];
...
function WelcomeOnboarding({ onDismiss }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "1rem" }}>
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 18, width: "100%", maxWidth: 460, padding: "2.5rem", textAlign: "center", position: "relative" }}>
        <button onClick={onDismiss} style={{ position: "absolute", top: 16, right: 16, background: "transparent", border: "none", color: T.faint, cursor: "pointer", fontSize: 18 }}>×</button>

        {/* Step indicators */}
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginBottom: "2rem" }}>
          {WELCOME_STEPS.map((_, i) => (
            <div key={i} style={{ width: i === step ? 24 : 8, height: 8, borderRadius: 4, background: i <= step ? T.accent : T.faint, transition: "all 0.3s" }} />
          ))}
        </div>

        <div style={{ fontSize: 40, marginBottom: "1rem" }}>{current.icon}</div>
        <div style={{ color: T.text, fontSize: 20, fontWeight: 700, marginBottom: "0.875rem" }}>{current.title}</div>
        <div style={{ color: T.muted, fontSize: 14, lineHeight: 1.75, marginBottom: "2rem" }}>{current.body}</div>

        <button onClick={() => isLast ? onDismiss() : setStep(s => s + 1)}
          style={{ background: T.accent, color: "#0d1117", border: "none", borderRadius: 10, padding: "12px 32px", cursor: "pointer", fontSize: 14, fontWeight: 700, width: "100%" }}>
          {current.cta}
        </button>
...
function Sidebar({ view, setView, onAddClient, onSearch, brand = DEFAULT_BRAND, activity = [], onNavigate }) {
  ...
  return (
    <div style={{ width: 220, background: T.surface, borderRight: `1px solid ${T.border}`, height: "100vh", display: "flex", flexDirection: "column", position: "fixed", left: 0, top: 0 }}>
      <div style={{ padding: "1.25rem 1.25rem 1rem", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div style={{ minWidth: 0 }}>
          {brand.logoUrl ? (
            <img src={brand.logoUrl} alt={brand.name} style={{ height: 28, maxWidth: 140, objectFit: "contain", marginBottom: 4 }} onError={e => e.target.style.display = "none"} />
          ) : (
        ...
        <NotificationCentre activity={activity} onNavigate={onNavigate || setView} />
      </div>
      ...
      <div style={{ padding: "0 0.75rem 0.5rem" }}>
        <a href="/features.html" target="_blank" rel="noopener noreferrer"
          style={{ display: "flex", alignItems: "center", gap: 6, width: "100%", background: "transparent", border: `1px solid ${T.border}`, borderRadius: 8, padding: "7px", cursor: "pointer", fontSize: 11, color: T.muted, textDecoration: "none", justifyContent: "center" }}>
      ...
      <div style={{ padding: "0 0.75rem 0.5rem" }}>
        <button onClick={handleLogout}
          style={{ width: "100%", background: "transparent", border: `1px solid ${T.border}`, borderRadius: 8, padding: "8px", cursor: "pointer", fontSize: 12, color: T.muted, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          <span>↩</span> Sign out
        </button>
      </div>
...
// ─── DASHBOARD ───────────────────────────────────────────────────────────────
function Dashboard({ setView, clients, campaigns, onDeleteClient, onEditClient, activity, loading, onAuditClient }) {
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {clients.map(c => (
              <div key={c.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "1.125rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: "0.75rem" }}>
                  {[["Sent", (c.messages||0).toLocaleString()], ["Replies", c.replies||0], ["Meetings", c.meetings||0]].map(([k, v]) => (
                ...
                <div style={{ display: "flex", gap: 5 }}>
                  <button onClick={() => setReportClient(c)} style={{ flex: 1, background: "transparent", color: T.muted, border: `1px solid ${T.border}`, borderRadius: 7, padding: "6px", cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                    <span>📊</span> Report
                  </button>
                  <button onClick={() => onAuditClient && onAuditClient(c)} style={{ flex: 1, background: "transparent", color: T.muted, border: `1px solid ${T.border}`, borderRadius: 7, padding: "6px", cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                    <span>📋</span> Activity
                  </button>
                  <button onClick={() => onEditClient && onEditClient(c)} style={{ flex: 1, background: T.accentBg, color: T.accent, border: `1px solid ${T.accent}44`, borderRadius: 7, padding: "6px", cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                    ✏ ICP
                  </button>
                  <button onClick={() => { if (window.confirm(`Remove ${c.name}?`)) onDeleteClient(c.id); }} style={{ background: "transparent", color: T.red, border: `1px solid ${T.red}33`, borderRadius: 7, padding: "6px 8px", cursor: "pointer", fontSize: 11 }}>✕</button>
                </div>
              </div>
            ))}
          </div>
...
const DEMO_SEARCH_RESULTS = [
  { linkedin_urn: "s101", name: "Jordan Price",   title: "VP of Sales",          company: "Veritas",    location: "Austin, TX",        degree: "2nd", initials: "JP", linkedin_url: "" },
  { linkedin_urn: "s102", name: "Hannah Cole",    title: "Head of Growth",       company: "Sprout",     location: "New York, NY",      degree: "2nd", initials: "HC", linkedin_url: "" },
  { linkedin_urn: "s103", name: "Dev Sharma",     title: "Founder & CEO",        company: "Kova Labs",  location: "San Francisco, CA", degree: "3rd", initials: "DS", linkedin_url: "" },
  { linkedin_urn: "s104", name: "Tess Andersen",  title: "Chief Revenue Officer",company: "MerakiPay",  location: "Chicago, IL",       degree: "2nd", initials: "TA", linkedin_url: "" },
  { linkedin_urn: "s105", name: "Marcus Obi",     title: "Founder & CEO",        company: "Qubit",      location: "London, UK",        degree: "3rd", initials: "MO", linkedin_url: "" },
  { linkedin_urn: "s106", name: "Claire Forster", title: "Sales Director",       company: "Lumio",      location: "Boston, MA",        degree: "2nd", initials: "CF", linkedin_url: "" },
  { linkedin_urn: "s107", name: "Ryan Nakamura",  title: "VP Business Dev",      company: "Lattice",    location: "Seattle, WA",       degree: "2nd", initials: "RN", linkedin_url: "" },
  { linkedin_urn: "s108", name: "Sofia Morales",  title: "Head of Sales",        company: "Deel",       location: "Miami, FL",         degree: "3rd", initials: "SM", linkedin_url: "" },
  { linkedin_urn: "s109", name: "Tom Bradley",    title: "Director of Sales",    company: "Notion",     location: "New York, NY",      degree: "2nd", initials: "TB", linkedin_url: "" },
  { linkedin_urn: "s110", name: "Anya Kapoor",    title: "VP Sales",             company: "Figma",      location: "San Francisco, CA", degree: "1st", initials: "AK", linkedin_url: "" },
  { linkedin_urn: "s111", name: "Liam Chen",      title: "Sales Manager",        company: "HubSpot",    location: "Dublin, Ireland",   degree: "2nd", initials: "LC", linkedin_url: "" },
...
function ImportModal({ onClose, onImport, clients = [] }) {
  ...
  const scoreLeads = async () => {
    ...
    try {
      const res = await fetch('/api/leads/score', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          leads: searchResults.map((r, i) => ({ id: r.linkedin_urn || String(i), name: r.name, title: r.title, company: r.company, location: r.location })),
          icp: searchFilters,
      ...
      const data = await res.json();
      const scoreMap = {};
      (data.scores || []).forEach(s => { scoreMap[s.id] = s; });
      ...
      searchResults.forEach((r, i) => {
        ...
        demo[r.linkedin_urn || String(i)] = { score: s, reason: s >= 80 ? "Strong title and industry match" : s >= 60 ? "Partial match on key criteria" : "Weak match — consider excluding" };
      });
  ...
  const toggleSearchResult = (urn) => setSearchSelected(s => { const n = new Set(s); n.has(urn) ? n.delete(urn) : n.add(urn); return n; });
  const toggleAllSearch = () => setSearchSelected(s => s.size === searchResults.length ? new Set() : new Set(searchResults.map(r => r.linkedin_urn)));

  const importSearchResults = () => {
    const selected = searchResults.filter(r => searchSelected.has(r.linkedin_urn));
    const newLeads = selected.map((r, i) => ({
      id: Date.now() + i, name: r.name, title: r.title, company: r.company,
      initials: r.initials, color: ["#58a6ff","#2dce98","#bc8cff","#d29922","#f85149"][i % 5],
      clientColor: "#2dce98", campaign: listName || "LinkedIn Search", client: "",
      pipelineStage: "prospecting", days: 0, status: "pending", unread: false, last: "Just now",
      messages: [], linkedin_url: r.linkedin_url, linkedin_urn: r.linkedin_urn,
  ...
  // Saved searches
  const [savedSearches, setSavedSearches] = useLocalStorage("rf_saved_searches", []);
  const saveCurrentSearch = () => {
    const name = prompt("Name this search:");
    if (!name) return;
    setSavedSearches(ss => [...ss, { id: Date.now(), name, filters: { ...searchFilters } }]);
    pushToast(`Search "${name}" saved`, "success");
  };
  const loadSavedSearch = (s) => { setFilter(s.filters); };
  const deleteSavedSearch = (id) => setSavedSearches(ss => ss.filter(s => s.id !== id));

  const startLookup = async () => {
    const urls = profileUrlsText.split("\n").map(s => s.trim()).filter(Boolean);
      ...
      const results = await Promise.all(urls.map(async (u) => {
        try {
          const res = await fetch('/api/linkedin/lookup', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ linkedin_url: u, account_id: client.unipileAccountId }),
          });
          const data = await res.json();
          if (data.ok) {
            return { id: u, url: u, ok: true, selected: true, ...data.profile };
          }
          return { id: u, url: u, ok: false, selected: false, name: u, title: "", company: "", location: "", error: data.error || "Lookup failed" };
        } catch (err) {
          return { id: u, url: u, ok: false, selected: false, name: u, title: "", company: "", location: "", error: err.message || "Lookup failed" };
        }
      }));
      setRows(results);
      setStage("preview");
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, width: "100%", maxWidth: stage === "preview" || (tab === "search" && searchResults.length > 0) ? 680 : 520, maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {stage === "input" && (
          <div style={{ display: "flex", gap: 0, padding: "0.875rem 1.5rem 0", borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
            {[["search", "🔍 Search LinkedIn"], ["linkedin", "Add by URL"], ["csv", "CSV Upload"]].map(([id, label]) => (
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "1.5rem" }}>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: "1rem" }}>
                {[
                  ...
                  <div key={key}>
                    <div style={{ color: T.muted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 5 }}>{label}</div>
                    <input style={{ ...inp }} placeholder={placeholder} value={searchFilters[key]} onChange={e => setFilter({ [key]: e.target.value })} onKeyDown={e => e.key === "Enter" && runSearch()} />
                  </div>
                ))}
              </div>

              <div style={{ marginBottom: "1.25rem" }}>
                <div style={{ color: T.muted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 5 }}>Connection degree</div>
                <div style={{ display: "flex", gap: 8 }}>
                  {[["", "All"], ["1st", "1st"], ["2nd", "2nd"], ["3rd", "3rd+"]].map(([val, label]) => (
                    <button key={val} onClick={() => setFilter({ degree: val })}
                      style={{ flex: 1, background: searchFilters.degree === val ? T.accentBg : T.card, color: searchFilters.degree === val ? T.accent : T.muted, border: `1px solid ${searchFilters.degree === val ? T.accent : T.border}`, borderRadius: 7, padding: "7px", cursor: "pointer", fontSize: 12, fontWeight: searchFilters.degree === val ? 700 : 400 }}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: "1.25rem" }}>
                {(searchFilters.title || searchFilters.industry || searchFilters.company || searchFilters.location) && (
                  <button onClick={saveCurrentSearch} style={{ background: "transparent", color: T.muted, border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px 10px", cursor: "pointer", fontSize: 11 }}>💾 Save this search</button>
                )}
                {savedSearches.length > 0 && (
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {savedSearches.map(s => (
                      <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 4, background: T.card, border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 8px" }}>
                        <button onClick={() => loadSavedSearch(s)} style={{ background: "transparent", border: "none", color: T.accent, cursor: "pointer", fontSize: 11, fontWeight: 600, padding: 0 }}>{s.name}</button>
                        <button onClick={() => deleteSavedSearch(s.id)} style={{ background: "transparent", border: "none", color: T.faint, cursor: "pointer", fontSize: 12, padding: 0, lineHeight: 1 }}>×</button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {searchResults.length > 0 && (
                <div>
                  {/* Results header */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
                    <div style={{ color: T.text, fontSize: 13, fontWeight: 600 }}>{searchResults.length} results — {searchSelected.size} selected</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={toggleAllSearch} style={{ background: "transparent", color: T.muted, border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11 }}>
                        {searchSelected.size === searchResults.length ? "Deselect all" : "Select all"}
                      </button>
                      <button onClick={scoreLeads} disabled={scoring}
                        style={{ background: scoring ? T.faint : T.accentBg, color: T.accent, border: `1px solid ${T.accent}44`, borderRadius: 6, padding: "4px 12px", cursor: scoring ? "default" : "pointer", fontSize: 11, fontWeight: 700 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 480, overflowY: "auto", marginBottom: "1rem" }}>
                    {searchResults.map((r, i) => {
                      ...
                      const scoreColor = scoreData ? (scoreData.score >= 80 ? T.accent : scoreData.score >= 60 ? T.yellow : T.red) : null;
                      return (
                        <div key={r.linkedin_urn} onClick={() => toggleSearchResult(r.linkedin_urn)}
                          style={{ display: "flex", alignItems: "center", gap: 10, background: sel ? T.accentDim : T.card, border: `1px solid ${sel ? T.accent + "44" : T.border}`, borderRadius: 9, padding: "0.75rem", cursor: "pointer", transition: "all 0.12s" }}>
                          <input type="checkbox" readOnly checked={sel} style={{ accentColor: T.accent, flexShrink: 0 }} />
                          <div style={{ width: 30, height: 30, borderRadius: "50%", background: ["#58a6ff","#2dce98","#bc8cff","#d29922","#f85149"][i % 5] + "22", color: ["#58a6ff","#2dce98","#bc8cff","#d29922","#f85149"][i % 5], display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, flexShrink: 0 }}>{r.initials}</div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ color: T.text, fontSize: 13, fontWeight: 600 }}>{r.name}</div>
                            <div style={{ color: T.muted, fontSize: 11 }}>{r.title} · {r.company}</div>
                            <div style={{ color: T.faint, fontSize: 10 }}>{r.location}{r.degree ? ` · ${r.degree} connection` : ""}</div>
                          </div>
                          {scoreData && (
                            <div style={{ textAlign: "center", flexShrink: 0 }}>
                              <div style={{ color: scoreColor, fontSize: 16, fontWeight: 900 }}>{scoreData.score}</div>
                              <div style={{ color: T.faint, fontSize: 9 }}>ICP score</div>
                            </div>
                          )}
                        </div>
                      );
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ color: T.muted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 5 }}>List name</div>
                    <input style={inp} placeholder="e.g. VP Sales — SaaS — US" value={listName} onChange={e => setListName(e.target.value)} />
                  </div>
                  <button onClick={importSearchResults} disabled={searchSelected.size === 0}
                    style={{ width: "100%", background: searchSelected.size > 0 ? T.accent : T.faint, color: searchSelected.size > 0 ? "#0d1117" : T.muted, border: "none", borderRadius: 8, padding: "11px", cursor: searchSelected.size > 0 ? "pointer" : "default", fontSize: 13, fontWeight: 700 }}>
          ...
          {tab === "linkedin" && stage === "input" && (
            <div>
              <div style={{ color: T.muted, fontSize: 13, marginBottom: "1.25rem", lineHeight: 1.6 }}>
                Paste one or more LinkedIn profile URLs (one per line). Each one is looked up via your connected LinkedIn account, so the lead comes in with real data — name, title, company — ready to actually send to.
              </div>

              {connectedClients.length === 0 ? (
                <div style={{ background: T.card, border: `1px solid ${T.yellow}44`, borderRadius: 8, padding: "0.875rem 1rem", marginBottom: "1.25rem", color: T.muted, fontSize: 12.5, lineHeight: 1.6 }}>
                  <strong style={{ color: T.yellow }}>No connected LinkedIn account.</strong> Connect a client's LinkedIn account in Settings before adding leads this way — the lookup runs through their connected account.
                </div>
              ) : (
                <div style={{ marginBottom: "1.125rem" }}>
                  <div style={{ color: T.muted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 5 }}>Look up using account</div>
                  <select style={inp} value={searchClientId} onChange={e => setSearchClientId(e.target.value)}>
                    {connectedClients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </div>
              ...
              <div style={{ marginBottom: "1rem" }}>
                <div style={{ color: T.muted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 6 }}>LinkedIn Profile URLs (one per line)</div>
                <textarea style={{ ...inp, minHeight: 110, resize: "vertical", fontFamily: "monospace", fontSize: 12.5 }}
                  placeholder={"https://www.linkedin.com/in/janedoe\nhttps://www.linkedin.com/in/johndoe"}
                  value={profileUrlsText} onChange={e => setProfileUrlsText(e.target.value)} />
              </div>
              ...
              <div style={{ background: T.accentDim, border: `1px solid ${T.accent}33`, borderRadius: 10, padding: "1rem" }}>
                <div style={{ color: T.accent, fontSize: 12, fontWeight: 700, marginBottom: 6 }}>How this works</div>
                {[
                  "Each URL is looked up individually via Unipile — real data, no guessing",
                  "Looking for a bulk audience instead of specific people? Use Search LinkedIn instead",
                  "Profiles that can't be resolved (private, invalid URL, etc.) will show an error and won't be imported",
                ].map(t => (
          ...
          {stage === "scraping" && (
            <div style={{ textAlign: "center", padding: "2rem 0" }}>
              ...
              <div style={{ color: T.text, fontWeight: 700, fontSize: 16, marginBottom: 6 }}>Looking up profiles…</div>
              <div style={{ color: T.muted, fontSize: 13 }}>Resolving real profile data via your connected LinkedIn account</div>
            </div>
          )}
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                ...
                {rows.some(r => r.ok) && (
                  <button onClick={toggleAll} style={{ background: T.card, color: T.muted, border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px 10px", cursor: "pointer", fontSize: 12 }}>
                    {rows.filter(r => r.ok).every(r => r.selected) ? "Deselect all" : "Select all"}
                  </button>
                )}
              </div>
              <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {rows.map(row => (
                      <tr key={row.id} onClick={() => row.ok && toggleRow(row.id)} style={{ cursor: row.ok ? "pointer" : "default", background: row.selected ? T.accentDim : "transparent", opacity: row.ok ? (row.selected ? 1 : 0.4) : 0.5 }}>
                        ...
                        <td style={{ ...td, fontWeight: 600 }}>{row.ok ? row.name : row.url}</td>
                        {row.ok ? (
                          <>
                          ...
                          <td colSpan={3} style={{ ...td, color: T.red, fontSize: 11.5 }}>{row.error}</td>
                        )}
                      </tr>
                    ))}
                  </tbody>
...
function Leads({ leads, setLeads, updateLeadsBulk, deleteLeadsBulk, logActivity, clients = [], campaigns = [] }) {
  ...
  const filtered = (filter === "all" ? leads : filter === "needs_review" ? leads.filter(l => l.sequenceStatus === "needs_review") : leads.filter(l => l.status === filter))
    .filter(l => {
      ...
      if (scoreFilter === "medium") return l.qualityScore != null && l.qualityScore >= 60 && l.qualityScore < 80;
      if (scoreFilter === "low")    return l.qualityScore != null && l.qualityScore < 60;
  ...
  return (
    <div>
      ...
      {selected.size > 0 && (
        <div style={{ position: "fixed", bottom: 24, left: 256, right: 36, zIndex: 100, background: T.surface, border: `1px solid ${T.accent}`, borderRadius: 10, padding: "0.75rem 1rem", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 -6px 20px rgba(0,0,0,0.35)" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: "0.75rem", flexWrap: "wrap" }}>
        <span style={{ color: T.muted, fontSize: 11, alignSelf: "center" }}>ICP Score:</span>
        {[["all", "All"], ["high", "🟢 80+"], ["medium", "🟡 60–79"], ["low", "🔴 <60"], ["unscored", "— Unscored"]].map(([val, label]) => (
          <button key={val} onClick={() => setScoreFilter(val)} style={{ background: scoreFilter === val ? T.accentBg : "transparent", color: scoreFilter === val ? T.accent : T.muted, border: `1px solid ${scoreFilter === val ? T.accent : T.border}`, borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11, fontWeight: scoreFilter === val ? 700 : 400 }}>{label}</button>
        ))}
      </div>
              ...
              <tr key={lead.id} style={{ cursor: "pointer", background: selected.has(lead.id) ? T.accentDim : "transparent" }}>
                ...
                <td style={td}>
                  {lead.qualityScore != null ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <span style={{ color: lead.qualityScore >= 80 ? T.accent : lead.qualityScore >= 60 ? T.yellow : T.red, fontSize: 14, fontWeight: 800 }}>{lead.qualityScore}</span>
                      <div style={{ flex: 1, background: T.faint, borderRadius: 2, height: 3, width: 40 }}>
                        <div style={{ background: lead.qualityScore >= 80 ? T.accent : lead.qualityScore >= 60 ? T.yellow : T.red, height: 3, borderRadius: 2, width: `${lead.qualityScore}%` }} />
                      </div>
                    </div>
                ...
                <td style={td}>
                  {lead.sequenceStatus === "needs_review" ? (
                    <span title={lead.reviewReason || "Needs review before this lead can run"} style={{ background: "rgba(210,153,34,0.15)", color: T.yellow, border: `1px solid ${T.yellow}40`, fontSize: 12.5, fontWeight: 700, padding: "3px 9px", borderRadius: 4, letterSpacing: "0.04em", whiteSpace: "nowrap", cursor: "help" }}>
                      ⚠ Needs Review
                    </span>
                ...
                <div key={lead.id} draggable
                  ...
                  {lead.qualityScore != null && (
                    <div style={{ marginTop: 5, display: "flex", alignItems: "center", gap: 5 }}>
                      <span style={{ color: lead.qualityScore >= 80 ? T.accent : lead.qualityScore >= 60 ? T.yellow : T.red, fontSize: 10, fontWeight: 800 }}>ICP {lead.qualityScore}</span>
                      <div style={{ flex: 1, background: T.faint, borderRadius: 2, height: 3 }}>
                        <div style={{ background: lead.qualityScore >= 80 ? T.accent : lead.qualityScore >= 60 ? T.yellow : T.red, height: 3, borderRadius: 2, width: `${lead.qualityScore}%` }} />
                      </div>
                    </div>
                  )}
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user && !settings.fromName) {
        set({ fromName: user.user_metadata?.full_name || user.email?.split("@")[0] || "" });
      }
    });
  }, []);
...
function FlowBuilder({ campaign, onClose, savedFlow, onSave, voiceProfile = DEFAULT_VOICE_PROFILE }) {
  ...
  const loadTemplate = (id) => {
    const tmpl = TEMPLATE_FLOWS[id];
    if (!tmpl) return;
    if (flow.length > 0 && !window.confirm("This replaces the current flow with the template. Continue?")) return;
    setFlow(JSON.parse(JSON.stringify(tmpl))); // deep copy so editing doesn't mutate the shared template
    setEditNode(null); setSelectedId(null);
    ...
    <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 300, display: "flex", flexDirection: "column", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "0.875rem 1.5rem", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 6, position: "relative" }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setTemplateMenuOpen(o => !o)}
              style={{ background: "rgba(45,206,152,0.10)", color: T.green, border: `1px solid ${T.green}44`, borderRadius: 7, padding: "6px 10px", cursor: "pointer", fontSize: 11, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
              ⑃ Load template ▾
            </button>
            {templateMenuOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 9, padding: 6, minWidth: 240, boxShadow: "0 8px 24px rgba(0,0,0,0.35)", zIndex: 10, maxHeight: 320, overflowY: "auto" }}>
                {TEMPLATES.map(t => (
                  <button key={t.id} onClick={() => loadTemplate(t.id)}
                    ...
                    <div style={{ color: T.muted, fontSize: 11, marginTop: 1 }}>{t.bestFor}</div>
                  </button>
                ))}
              </div>
            )}
...
export default function App() {
  ...
  const [welcomeDismissed, setWelcomeDismissed] = useLocalStorage("rf_welcomed", false);
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: T.bg, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      {auditClient && <AuditLogModal client={auditClient} activity={activity} onClose={() => setAuditClient(null)} />}
